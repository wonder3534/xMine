...
function parseArticle(article) {
  ...
  const text = cleanTweetText(textRaw)
...
async function captureThreadToTarget(targetArticle) {
  ...
  // Strategy: Start from the top (first valid tweet)
  let mainIndex = -1
  for (let i = 0; i < all.length; i++) {
    // A valid tweet usually has a time element
  ...
  // If no valid start found, or target is the start
  ...
  // Safety check: target must be below main
  ...
  // Iterate from 1 (first reply) to end
